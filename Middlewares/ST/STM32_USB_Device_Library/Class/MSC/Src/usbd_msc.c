...
uint8_t USBD_MSC_Init(USBD_HandleTypeDef *pdev, uint8_t cfgidx)
{
  ...
  static USBD_MSC_BOT_HandleTypeDef inst;
  hmsc = &inst; //(USBD_MSC_BOT_HandleTypeDef *)USBD_malloc(sizeof(USBD_MSC_BOT_HandleTypeDef));
    ...
    case USB_REQ_TYPE_CLASS:
      switch (req->bRequest)
      {
        case BOT_GET_MAX_LUN:
          ...
          {
            hmsc->max_lun = (uint32_t)pdev->pClassSpecificInterfaceMSC->GetMaxLun();
