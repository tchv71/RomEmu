...
uint8_t* USBD_MSC_CDC_GetCfgDesc(uint16_t *length);
...
static uint8_t USBD_MSC_CDC_CfgDesc[USB_MSC_CDC_CONFIG_DESC_SIZ] =
  ...
  LOBYTE(USB_FS_MAX_PACKET_SIZE), HIBYTE(USB_FS_MAX_PACKET_SIZE), 0x00, /*Polling interval in milliseconds */
  /* 07 bytes */
  ...
  HIBYTE(CDC_DATA_PACKET_SIZE), 0x00, /* bInterval: ignore for Bulk transfer */
  /* 07 bytes */
  ...
  0x02, /* bmAttributes: Bulk */
  LOBYTE(CDC_DATA_PACKET_SIZE), /* wMaxPacketSize: */
  HIBYTE(CDC_DATA_PACKET_SIZE), 0x00, /* bInterval */
  /* 07 bytes */
...
static uint8_t USBD_MSC_CDC_DeviceQualifierDesc[USB_LEN_DEV_QUALIFIER_DESC] =
  ...
  USB_FS_MAX_PACKET_SIZE, 0x01, 0x00, };
 ...
 * @retval status
 */
uint8_t USBD_MSC_CDC_Setup(USBD_HandleTypeDef *pdev,
				  USBD_SetupReqTypedef *req)
{
  ...
  if (((req->bmRequest & USB_REQ_RECIPIENT_MASK) == USB_REQ_RECIPIENT_INTERFACE
      && req->wIndex == MSC_INTERFACE_IDX)
      || ((req->bmRequest & USB_REQ_RECIPIENT_MASK)
	  == USB_REQ_RECIPIENT_ENDPOINT && ((req->wIndex & 0x7F) == MSC_EP_IDX)))
  {
